...
const zh: Translations = {
	...
	'auth.notConfigured': 'API Key 未配置，请在命令面板运行 "OpenCode: 设置 API Key"。',
	...
	'error.opencode.zenOnlyModel':
		'该模型不在 OpenCode Go 订阅内，仅在 Zen 按量付费端点提供，需要 Zen 余额。请改用 Go 订阅内的模型（GLM、Kimi、MiniMax、Grok 4.5 等），或按上方链接充值 Zen 余额。',
	'model.zenPaygOnly': '仅限 Zen 按量付费（不含在 Go 订阅内）',
...
const en: Translations = {
	...
	'auth.notConfigured':
		'OpenCode API key not configured. Run "OpenCode: Set API Key" from the Command Palette.',
	...
	'error.opencode.zenOnlyModel':
		'This model is not included in the OpenCode Go subscription — it is only served on the Zen pay-as-you-go endpoint and requires Zen credit. Switch to a Go-subscription model (GLM, Kimi, MiniMax, Grok 4.5, …) or add Zen credit via the link above.',
	'model.zenPaygOnly': 'Zen pay-as-you-go only (not in the Go subscription)',
