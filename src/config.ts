...
import { getDynamicModels } from './provider/opencode-models';
...
const CUSTOM_MODEL_DETAIL = 'Custom OpenCode-compatible model';
const CUSTOM_MODEL_MAX_INPUT_TOKENS = 200_000;
const CUSTOM_MODEL_MAX_OUTPUT_TOKENS = 131_072;

const UTILITY_GO_DEFAULT_API_MODEL = 'deepseek-v4-flash';
const UTILITY_ZEN_DEFAULT_API_MODEL = 'deepseek-v4-flash-free';
 ...
 *   2. `endpoint` preset (including legacy GLM preset strings mapped to OpenCode)
 *   3. Active OpenCode plan default (`opencode-go` / `opencode-zen`)
 */
export function getBaseUrl(): string {
	...
	return resolveEndpointBaseUrl(getEndpoint());
}
...
/**
 * Active endpoint preset.
 *
 * Resolution order:
 *   1. Explicit `endpoint` setting (legacy GLM values are migrated on read)
 *   2. Legacy `apiProtocol=anthropic` without `endpoint` → plan's Anthropic preset
 *   3. Active OpenCode plan default
 */
export function getEndpoint(): EndpointPreset {
	...
	if (hasExplicitLegacyApiProtocol()) {
		return getOpencodePlan() === 'zen' ? 'opencode-zen-anthropic' : 'opencode-go-anthropic';
	}
...
function hasExplicitLegacyApiProtocol(): boolean {
	const config = vscode.workspace.getConfiguration(CONFIG_SECTION);
	const inspection = config.inspect<string>('apiProtocol');
	return (
...
export function getApiProtocol(): ApiProtocol {
	...
	return resolveEndpointProtocol(preset);
}
...
function normalizeEndpointPreset(value: unknown): EndpointPreset | undefined {
	return normalizeLegacyEndpointPreset(value);
}

export function getApiModelId(vscodeModelId: string): string {
	const override = getModelIdOverrides()[vscodeModelId]?.trim();
	if (isUtilityModelId(vscodeModelId)) {
		const plan = getOpencodePlan();
		if (plan === 'zen') {
			if (override && override !== UTILITY_GO_DEFAULT_API_MODEL) {
				return override;
			}
			return UTILITY_ZEN_DEFAULT_API_MODEL;
		}
	}
	return override || vscodeModelId;
}

function isUtilityModelId(modelId: string): boolean {
